...
- INMARSAT ISatPhone: Make sure you have a INMARSAT ISatPhone device with an active subscription. 
...
The messages received on the ISatPhone device appear as follows (based on the request above):
...
To decode these messages efficiently, open them via the INMARSAT Earthmate App on an iPad. Then, copy and paste the messages into the Decoder Jupyter Notebook, for instance, using the Carnets App. Once decoded, you can view the resulting GRIB-file with a GRIB viewer app, such as LuckGrib.
...
NOTE: In branch "chatgpt_conversation_module", I've added a ChatGPT module that leverages the OpenAI GPT-4 model to enable the INMARSAT user to ask any questions while being on open seas. 
